...
public class UserController {
	...
	*/
	...
	@RequestMapping(params="method=register",method={RequestMethod.POST})
	public void registerUser(User user,String userMobile,HttpServletRequest request,HttpServletResponse response){
...
//		}
		//4.加密密码
		String MD5Pwd = MD5.getMD5(user.getUserVerifyCode()+user.getUserPassword());
	...
	/*
	@RequestMapping(params="method=register",method={RequestMethod.POST})
	public void registerUser(String userPhone,String userVcode,String userPassword,
			String idcard,String userLicense,
...
//		}
		User user=new User();
		//4.加密密码
		String MD5Pwd = MD5.getMD5(userVcode+userPassword);
		if (MD5Pwd.length() == 32) {// 符合密码要求，传过来的是32位加密后的密码数据
			// 6.获取的手机号码不为空时，设置手机号码为注册的号码
			if (userPhone != null) {// 获取到手机号
				user.setUserMobile(userPhone);
				user.setUserPassword(MD5Pwd);
				user.setUserIDCard(idcard);
				user.setUserLicense(userLicense);
				user.setUserType(3);
				user.setUserVerifyCode(userVcode);
				// 调用业务方法，注册用户
				user = userService.registerUser(user);	
				// 返回客户端json数据
				jsonObject.put("registerState", "200");// 注册成功
